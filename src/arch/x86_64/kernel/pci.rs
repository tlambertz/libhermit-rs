...
impl PciAdapter {
	...
	pub fn memory_map_bar(&self, index: u8) -> Option<(usize, usize)> {
		// Find bar struct from index and check if it is a memory bar
		let mut iter = self.base_addresses.iter();
		let bar = loop {
			match iter.next() {
				Some(PciBar::IO(bar)) => {
					if bar.index == index {
		...
		/*
		We do not need to be careful about the physical memory overlapping, this is handeled by the bios/bootloader.
		rustyhermit gets for example the following multiboot memory info from qemu:

			Addr       Size      Type
		0x00000000 0x0009fc00 Available
		0x0009fc00 0x00000400 Reserved
		0x000f0000 0x00010000 Reserved
		0x00100000 0x3fedf000 Available
		0x3ffdf000 0x00021000 Reserved
		0xb0000000 0x10000000 Reserved
		0xfed1c000 0x00004000 Reserved
		0xfeffc000 0x00004000 Reserved
		0xfffc0000 0x00040000 Reserved

		On the same invocation, virtio-pci devices have bars at
		0xfe000000
		0xfe004000
		-> neither in Available nor in Reserved space? But since not in available we are safe.
		*/

		// Map bar into RW^X virtual memory
		let physical_address = bar.addr;
		let virtual_address = ::mm::map(physical_address, bar.size, true, false, false);
