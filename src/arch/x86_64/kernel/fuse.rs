...
impl<T: FuseInterface> FuseDriver<T> {
		...
		R: FuseOut + core::fmt::Debug,
	{
		...
		if let Some(mut rsp) = rsp {
			...
			self.0.borrow_mut().send_recv_buffers_blocking(&to_host, &from_host);
			trace!("Got Fuse Reply: {:?}", rsp);
			Some(rsp)
		} else {
			self.0.borrow_mut().send_recv_buffers_blocking(&to_host, &[]);
			None
		}
...
impl<T: FuseInterface> PosixFile for FuseFile<T> {
	...
	fn fsync(&mut self) -> Result<(), FileError> {
		...
		let _rsp = self.driver.send_command(Cmd {
				cmd,
				header: cmdhdr,
