...
use super::fuse_h::*;
use crate::syscalls::vfs::{Stat, FileError, FilePerms, PosixFile, PosixFileSystem, SeekWhence};
use alloc::boxed::Box;
use alloc::rc::Rc;
use alloc::vec::Vec;
...
pub trait FuseInterface {
	fn send_recv_buffers_blocking(&mut self, to_host: &[&[u8]], from_host: &[&mut [u8]]) -> Result<(), ()>;
}
...
impl<T: FuseInterface> FuseDriver<T> {
		...
		R: FuseOut + core::fmt::Debug,
	{
		...
		// Send the buffers
		self.0.borrow_mut().send_recv_buffers_blocking(&to_host, &from_host).map_err(|_| FileError::EIO)?;
		...
		R: FuseOut + core::fmt::Debug,
	{
		...
		// Send the buffers
		self.0.borrow_mut().send_recv_buffers_blocking(&to_host, &[]).map_err(|_| FileError::EIO)
	}
...
impl<T: FuseInterface> FuseFile<T> {
	...
	fn read_dax(&mut self, buf: &mut [u8]) -> Result<usize, FileError> {
		...
		let read_bytes = len;
		trace!("Starting dax copy. {:p}, {:#x}, {:p}, {:#x}", buf.as_ptr(), buf.len(), cached.as_ptr(), cached.len());

		// Use non-prefetching memcpy implementation
		//buf[..read_bytes].copy_from_slice(&cached[..len]);
		unsafe{ memcpy_noprefetch(buf.as_mut_ptr(), cached.as_ptr(), len); }

		trace!("read_dax output: {:?} ....", &buf[..core::cmp::min(16, buf.len())]);
		Ok(read_bytes)
	}
	...
	fn write_dax(&mut self, buf: &[u8]) -> Result<usize, FileError> {
		...
		trace!("write_dax({:x}) from offset {:x} [{:x}]", len, self.offset, self.attr.size);
		...
		trace!("copying data to slice! from {:p} to {:p} of len {:x}",  buf.as_ptr(), cached.as_ptr(), len);
		
		// Use non-prefetching memcpy implementation
		//cached[..len].copy_from_slice(buf);
		unsafe{ memcpy_noprefetch(cached.as_mut_ptr(), buf.as_ptr(), len); }
	...
	fn setupmapping(&mut self, 
		fh: u64,
		foffset: u64,
		len: u64,
		flags: u64,
		moffset: u64) -> Result<(), FileError>
	{
...
impl<T: FuseInterface> PosixFile for FuseFile<T> {
	...
	fn fsync(&mut self) -> Result<(), FileError> {
		...
		let _rsp = self.driver.handle_request(Cmd {
				cmd,
				header: cmdhdr,
				extra_buffer: FuseData(None),
			},
